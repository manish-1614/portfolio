...
export function ProductionSystemsSection() {
  ...
  return (
    <section id="systems" className="py-24 px-4 md:px-8 max-w-6xl mx-auto border-t border-slate-800/80">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {architecturalPillars.map((pillar, idx) => (
          <div
            key={idx}
            className="rounded-2xl border border-slate-800 bg-slate-900/30 backdrop-blur-sm p-6 md:p-8 flex flex-col justify-between space-y-6 hover:border-slate-700/80 transition-all shadow-lg"
          >
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs font-mono text-slate-500">
                <span>0{idx + 1} // ARCHITECTURE</span>
                <span className="text-indigo-400 font-semibold">{pillar.tech.split('//')[0]}</span>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-800/80">
                <p className="text-xs text-slate-400 leading-relaxed">
                  <strong className="text-slate-200">The Problem: </strong>
                  {pillar.problem}
                </p>
                <p className="text-xs text-slate-300 leading-relaxed">
                  <strong className="text-white">Engineering: </strong>
                  {pillar.solution}
                </p>
              </div>
            </div>
